...
# R2 fails on an otherwise-successful run; live incident 2026-08-07 on the
# brain/git boxes) falls through to `restic init`, whose "already initialized"
# Fatal kills every subsequent run until someone unlocks by hand. unlock only
# removes locks whose owning process is dead; on first run the repo doesn't
# exist yet and `|| true` keeps that from aborting the run that creates it.
