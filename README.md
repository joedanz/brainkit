...
| **Corrections** | `People/<id>/Corrections/` — when someone tells their agent it got something wrong, the agent writes the rule down; once that person confirms it in their dashboard, it becomes part of their generated protocol on the next compile, so the agent loads it every turn instead of forgetting it. Personal by default; one worth sharing goes to the team through the same human-approved queue |
| **Retrieval telemetry** | `.brain/retrieval-stats.json` — counts of what search returned, so an index that never built or a query that always comes back empty is visible instead of silent. Counts only, never the words anyone typed. An opt-in raw log is available for retrieval design work and reports its own state while it runs |
...
- **Revoking access does not un-deliver what was already read.** A revoke removes the space from that person's next compile, but their vault is a git repo: notes they could read while granted stay in its history, and a fresh clone carries that history. Revocation governs the future, not recall — rotate anything whose value survives being read, and to purge the server's copy, delete their compiled vault and recompile (it rebuilds with a fresh history). A clone already on their laptop is beyond any server's reach. [Details →](https://brainkit-docs.vercel.app/concepts/spaces-and-permissions#what-a-revoke-does-not-undo)
