---
title: One brain for the company
description: brainkit builds your company a shared brain on plain-markdown vaults, where privacy is enforced by construction rather than by prompt.
icon: brain
---

**brainkit** is the tool. A **company brain** is what you build with it: one master vault of shared knowledge, plus a filtered copy for every person.

Every employee gets a private, Obsidian-compatible knowledge vault holding only the notes they may read — kept current by whatever they connect to it, from the `brain` CLI to any AI tool over MCP. The company gets a shared brain that grows from that knowledge — **without private data ever leaking across employees**.
...
flowchart TB
    ...
    In["Intake<br/>email · chat · voice · webhooks · uploads"]
...
- **Intake** — notes reach a person's Inbox through `brain ingest`, the safe server-side primitive that email, chat, voice, and upload channels wrap — and that `brain webhook` serves over signed HTTP for services that push (Fathom, Zapier, Composio triggers). It can only ever write into that one person's space.
