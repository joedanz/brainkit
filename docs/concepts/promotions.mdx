...
<Steps>
  ...
  <Step title="Approve or reject">
    ...
    must be a person id from `_meta/org.yaml` **who holds `role: admin`** — a
    blank name, an unknown one, or a real colleague without the role is
    refused, so `approved-by` always points at someone entitled to publish.
    Rejections move to `rejected/` with the reason — a
...
The approver is checked the same way, at every layer: the core `approve` call, the dashboard's API, and the CLI all refuse a blank approver, one that isn't in the org roster, and one without `role: admin`. The rule lives in a single function — `may_approve` in `brain/promotions.py`, the promotions counterpart to `may_decide` for [shares](/concepts/spaces-and-permissions#sharing-a-space) — so the surfaces cannot drift apart. However an approval arrives, the `approved-by` line resolves to a real person entitled to publish.

<Callout type="info">
  Admin-only applies to **every** target, `Teams/<team>/` included. Delegating
  team-space promotions to that team's lead — the way share requests already
  route — is a deliberate future relaxation, not today's behavior. Approval
  starts restrictive and opens on purpose, never by omission.
</Callout>
