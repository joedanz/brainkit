...
<Callout type="warning">
  Promotions into the shared space are **never** decidable in-vault — not even by an admin. They stay at the dashboard, where the reviewer sees the full diff and a warning naming exactly who will be able to read the result. That is the whole reason the human gate exists; the seam is scoped to team spaces so it never bypasses it.
</Callout>
