...
<Callout type="info">
  `Shares.md` is a **pull** surface: it is always current, but nothing tells anyone to open it. A person learns a decision is waiting on them only if they look, and an admin only if they open the dashboard. Two ready-made [cron prompts](https://github.com/joedanz/brainkit/blob/main/deploy/agents-box/cron-prompts/) close that loop — a per-person nudge that reports what is waiting and records the decision its human gives, and an admin queue nudge that reports depth and age without deciding anything. Both stay silent when nothing is pending, and neither adds authority: the seam and its server-side eligibility re-check are unchanged.
</Callout>
