...
<Steps>
  <Step title="Draft">
    ...
    standing processes in `Company/Playbook/` — never an existing note.
...
The same check refuses a target that **already exists**. An approval writes the whole note to the target path; if that path held a curated file, approving would silently replace it. Failing closed keeps promotions strictly additive — to resolve the conflict, edit the pending file's `target-path` to a fresh filename and approve again. This is exactly how the [`Company/Intel/` wiki](/guides/getting-things-in#articles-and-links-the-intel-wiki) updates an existing page: the agent promotes a dated addendum (`<Name> — updates YYYY-MM.md`) to a fresh path, and the approver folds it into the main page and deletes the addendum in the same commit.
