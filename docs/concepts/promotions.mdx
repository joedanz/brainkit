...
<Steps>
  ...
  <Step title="Approve or reject">
    ...
    must be entitled to approve *this* promotion — an admin, or a lead of
    the team whose space it targets (see [Who approves](#who-approves)) — a
    blank name, an unknown one, or an ineligible colleague is refused, so
    `approved-by` always points at someone entitled to publish. Rejections
    move to `rejected/` with the reason — a training signal for what this
    company considers shareable.
...
The approver is checked the same way, at every layer: the core `approve` call, the dashboard's API, and the CLI all refuse a blank approver, one that isn't in the org roster, and one not entitled to approve that target (see [Who approves](#who-approves)). The rule lives in a single function — `may_approve` in `brain/promotions.py`, the promotions counterpart to `may_decide` for [shares](/concepts/spaces-and-permissions#sharing-a-space) — so the surfaces cannot drift apart. However an approval arrives, the `approved-by` line resolves to a real person entitled to publish.

## Who approves

| Target | Approver |
|---|---|
| `Teams/<team>/…` | anyone with `role: lead` on that team, or an admin |
| `Company/…` (the shared space) | an admin, at the dashboard or `brain promotions approve` |
| `Clients/<name>/…` (entity spaces) | an admin |

The rule lives in one function — `may_approve` in `brain/promotions.py`, the promotions counterpart to `may_decide` for [shares](/concepts/spaces-and-permissions#sharing-a-space) — and every surface calls it.

### Deciding from your own vault

A team lead does not need the dashboard. Every compile generates a **Promotions awaiting your decision** section into their `People/<lead>/Shares.md`, listing each pending promotion into a team they lead — with what would be published: the body for `create`/`append`, a live unified diff for `patch`, capped at 4 KB with a notice pointing at `brain promotions show <id>` when truncated. Each item states who would be able to read it once approved, the same audience the dashboard card warns about.

Leading a team is authority over a path, not access to it: if the rules in `spaces.yaml` say the lead cannot *read* the target space — an exact rule shadowing the `Teams/*` wildcard, say — the item is left out entirely. Nothing is rendered into a vault that the space's own permissions keep it out of; those promotions are decided at the dashboard. The lead (or their agent, recording a decision the lead made) writes:

```
People/<lead>/PromotionApprovals/<promo-id>.md
---
...
The next [`brain cycle`](/reference/cli#brain-cycle) applies it — re-checking eligibility server-side, refusing with an Inbox note when the promotion is already decided, targets a team the decider doesn't lead, or a rejection is missing its reason. A forged `owner:` is treated as tampering. Every applied decision is archived with `via: delegated`, committed under the decider's identity, and surfaced by [`brain doctor`](/reference/cli#brain-doctor) for 30 days.

<Callout type="warning">
  Promotions into the shared space are **never** decidable in-vault — not even by an admin. They stay at the dashboard. That carve-out is enforced twice: the cycle's sweep refuses the note, and `approve()` itself refuses any delegated decision on a shared-space target, so a pending file edited between the two still cannot publish company-wide. The seam is scoped to team spaces so it never bypasses the human gate.
</Callout>
...
<Callout type="info">
  `Shares.md` is a **pull** surface: it is always current, but nothing tells anyone to open it. A person learns a decision is waiting on them only if they look, and an admin only if they open the dashboard. Two ready-made [cron jobs](https://github.com/joedanz/brainkit/blob/main/deploy/agents-box/cron-jobs/) close that loop — a per-person nudge that reports what is waiting and records the decision its human gives, and an admin queue nudge that reports depth and age without deciding anything. Both print nothing when nothing is pending — so nothing is delivered — and neither adds authority: the seam and its server-side eligibility re-check are unchanged.
</Callout>
