...
It exposes four read-only tools: `brain_search` (hybrid search, with an
...
and `brain_recent` (recently changed notes, from the vault's git history).
See the [CLI reference](/reference/cli) for `brain index`, `brain search`,
and `brain mcp`.
