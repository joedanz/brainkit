...
An agent in a compiled slice cannot write the entity tree (Clients by
default) or _meta/. It drops a *request* in its own writable
People/<pid>/ClientRequests/ space; the
...
def requests_rel(config: VaultConfig, person_id: str) -> str:
    return f"People/{person_id}/{config.requests_folder}"
...
def request_client(
    ...
    to master, where materialize_clients provisions the entity space."""
    ...
    req_rel = requests_rel(config, person_id)
    ancestor = root
    ...
    dir_ = root / req_rel
    base = _slug(name) or config.entity
...
def append_client_grant(
    spaces_path: Path,
    client_name: str,
    owner_id: str,
    config: VaultConfig = VaultConfig(),
) -> bool:
    """Append an exact owner-bound <entities>/<Name> rule. Text append preserves
...
def _seed_note(body: str, owner: str, source: str, date: str, entity: str = "client") -> str:
    return (
        "---\n"
        f"entity: {entity}\n"
        f"created-by: {owner}\n"
...
def materialize_clients(
    master: Path, org: Org, today: str, config: VaultConfig | None = None
) -> list[ClientProvision]:
    """Provision one <entities>/<Name> space per pending request, server-side.
    ...
    for req in sorted(master.glob(f"People/*/{config.requests_folder}/*.md")):
        if req.is_symlink():
            continue
        ...
        try:
            ...
            if not meta:
                continue
            try:
                name = normalize_client_name(
                    meta.get(config.name_key) or meta.get("client-name") or "")
            except ClientError:
            ...
            if exact is not None or folder:
                ...
                inbox_rel = _inbox_note(master, person_id, name, slug, today, entity=config.entity)
                req.unlink()
            ...
            # (4) create
            note.parent.mkdir(parents=True, exist_ok=True)
            note.write_text(_seed_note(body, person_id, source, today, entity=config.entity))
            append_client_grant(master / "_meta/spaces.yaml", name, person_id, config=config)
