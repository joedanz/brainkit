...
def _eval(expr: str):
    script = f'import {{ parseHash, noteHash }} from "{HASH.as_uri()}"; console.log(JSON.stringify({expr}));'
...
@pytest.mark.parametrize("hash_, expected", [
    ("#query", {"tab": "query", "note": None}),
    ("query", {"tab": "query", "note": None}),
    ("", {"tab": None, "note": None}),
    ("#", {"tab": None, "note": None}),
    ("#note=Clients%2FAcme.md", {"tab": "query", "note": "Clients/Acme.md"}),
    ("#note=Clients/Acme.md", {"tab": "query", "note": "Clients/Acme.md"}),
    ("#note=", {"tab": "query", "note": None}),
    ("#note=%E0%A4%A", {"tab": "query", "note": None}),   # malformed escape: query tab, nothing opened
])
